...
## 1. Title
- Feature name: ОМС
- Source feature name, if different: OMS document
- Feature layer / epic / product area: Layer 1 / Подготовка документов / Supporting

## 2. Purpose
This supporting system behavior lets the patient open OMS document item so that пациент хранит документ, который может понадобиться при визите. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 14; Feature layer = Layer 1; Epic = Подготовка документов; Epic flow = Пациент хранит документ, который может понадобиться при визите; Feature = ОМС; Epic path priority = Supporting.
- Normalized feature name: OMS document
...
| Primary action | User performs the main action: open OMS document item. | Patient | User selection, query, upload, status change, or request payload as applicable. | Checks whether an oms document is already attached. | Shows upload/view/replace actions according to permissions. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Oms item contributes to document readiness if configured as required. |
...
2. System validates access and loads the minimum required data for ОМС.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open OMS document item.
...
- Description: Load only data the current actor is allowed to access for ОМС.
...
- Description: Support the core behavior where the user can open OMS document item.
...
|-------|---------|------------|----------------|
| oms_document_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| oms_document_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| oms_document_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/014-oms-document.md
