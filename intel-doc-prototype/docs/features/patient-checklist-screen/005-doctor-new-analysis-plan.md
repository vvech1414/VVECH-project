...
- Source feature name, if different: Doctor-created new analysis plan
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This feature lets the doctor create or update a new analysis plan so that врач формирует план новых анализов → пациент получает его в приложении. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 5; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Врач формирует план новых анализов → пациент получает его в приложении; Feature = План новых анализов от врача; Epic path priority = Core.
- Normalized feature name: Doctor-created new analysis plan
...
- Expected implementation target: Shared patient/doctor new analyses plan.
...
- Access control expectations: Doctor access must be scoped to the owning patient/preparation case; Doctor access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
- Auditability expectations: Plan creation, plan updates, result upload, linking, and doctor/admin views are audit-sensitive.
...
- Original vs structured data handling, if relevant: Uploaded result originals remain separate from OCR or structured extraction.
...
|-------|-------------|-------|-------|-----------------|---------------------|
| Entry | User reaches the feature from the preparation flow. | Doctor | Authenticated session and preparation case id. | Verify ownership or approved access before loading sensitive content. | Feature default or blocked state. |
| Primary action | User performs the main action: create or update a new analysis plan. | Doctor | User selection, query, upload, status change, or request payload as applicable. | Stores plan items linked to the preparation case. | Makes the plan visible to the patient with source doctor context. |
| Follow-through | User continues the preparation journey. | Doctor | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient reviews required new analyses. |
...
- Product surface: Shared patient/doctor new analyses plan.
- Upstream step: Doctor creates or patient receives a plan item for new analyses.
- Downstream step: Patient completes plan items and uploaded results become available in the preparation dossier.
...
- Internal storage or mocked service boundary for new analysis plan items and uploaded result documents.
...
### Happy path
1. Doctor enters the feature from the relevant preparation surface.
2. System validates access and loads the minimum required data for План новых анализов от врача.
3. System displays the default state with clear source/status information and available actions.
4. Doctor completes the primary action: create or update a new analysis plan.
...
### Empty
- Show a clear empty state when no relevant new analysis plan items and uploaded result documents exists; include the next safe action if available.
...
- Description: Load only data the current actor is allowed to access for План новых анализов от врача.
...
- Description: Support the core behavior where the user can create or update a new analysis plan.
...
|-------|---------|------------|----------------|
| doctor_new_analysis_plan_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| doctor_new_analysis_plan_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| doctor_new_analysis_plan_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisPlan, AnalysisPlanItem, AnalysisResultUpload, OriginalDocument, StructuredExtraction, DossierEntry.
...
- Suggested path: product/features/005-doctor-new-analysis-plan.md
- File role: Claude Code-ready feature specification for implementation planning and QA alignment.
- Related specs: Other specs in the same epic `План новых анализов` and adjacent source-order preparation features.
