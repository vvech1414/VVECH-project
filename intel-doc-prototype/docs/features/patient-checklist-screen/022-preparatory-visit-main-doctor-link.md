...
- Source feature name, if different: Preparatory visit to main doctor link
- Feature layer / epic / product area: Layer 2 / Запись к другим врачам для подготовки / Core

## 2. Purpose
This supporting system behavior lets the patient view preparatory visit relationship so that пациент понимает, зачем этот визит нужен в общем сценарии подготовки. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 22; Feature layer = Layer 2; Epic = Запись к другим врачам для подготовки; Epic flow = Пациент понимает, зачем этот визит нужен в общем сценарии подготовки; Feature = Связка “подготовительный визит → основной врач”; Epic path priority = Core.
- Normalized feature name: Preparatory visit to main doctor link
...
| Primary action | User performs the main action: view preparatory visit relationship. | Patient | User selection, query, upload, status change, or request payload as applicable. | Links related specialist visit to the main doctor preparation case. | Shows why this visit is part of preparation without clinical claims. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient sees how it affects preparation progress. |
...
2. System validates access and loads the minimum required data for Связка “подготовительный визит → основной врач”.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: view preparatory visit relationship.
...
- Description: Load only data the current actor is allowed to access for Связка “подготовительный визит → основной врач”.
...
- Description: Support the core behavior where the user can view preparatory visit relationship.
...
|-------|---------|------------|----------------|
| preparatory_visit_main_doctor_link_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| preparatory_visit_main_doctor_link_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| preparatory_visit_main_doctor_link_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/022-preparatory-visit-main-doctor-link.md
