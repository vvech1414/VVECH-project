...
- Source feature name, if different: Document readiness status
- Feature layer / epic / product area: Layer 1 / Подготовка документов / Core

## 2. Purpose
This supporting system behavior lets the patient view document preparation status so that пациент видит, насколько пакет документов готов. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 13; Feature layer = Layer 1; Epic = Подготовка документов; Epic flow = Пациент видит, насколько пакет документов готов; Feature = Статус готовности документов; Epic path priority = Core.
- Normalized feature name: Document readiness status
...
| Primary action | User performs the main action: view document preparation status. | Patient | User selection, query, upload, status change, or request payload as applicable. | Computes readiness from required checklist item states. | Shows ready, incomplete, or attention-needed status. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient continues missing items or proceeds if allowed. |
...
2. System validates access and loads the minimum required data for Статус готовности документов.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: view document preparation status.
...
- Description: Load only data the current actor is allowed to access for Статус готовности документов.
...
- Description: Support the core behavior where the user can view document preparation status.
...
|-------|---------|------------|----------------|
| document_readiness_status_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| document_readiness_status_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| document_readiness_status_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/013-document-readiness-status.md
