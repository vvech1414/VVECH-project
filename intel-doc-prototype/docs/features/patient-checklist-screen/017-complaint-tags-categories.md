# Теги / категории жалоб

## 1. Title
- Feature name: Теги / категории жалоб
- Source feature name, if different: Complaint tags and categories
- Feature layer / epic / product area: Layer 1 / Жалобы / Core

## 2. Purpose
This supporting system behavior lets the patient choose complaint tags/categories so that пациент структурирует свои жалобы в удобной форме. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 17; Feature layer = Layer 1; Epic = Жалобы; Epic flow = Пациент структурирует свои жалобы в удобной форме; Feature = Теги / категории жалоб; Epic path priority = Core.
- Normalized feature name: Complaint tags and categories
...
- Expected implementation target: Patient-side complaints preparation block.
...
- Access control expectations: Patient access must be scoped to the owning patient/preparation case; Doctor with explicit access to the preparation case access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
- Auditability expectations: Doctor/admin view and complaint changes should be traceable; patient edits should keep updated timestamps.
...
- Original vs structured data handling, if relevant: Patient-entered text is source content; any structured tags are separate derived metadata.
...
| Primary action | User performs the main action: choose complaint tags/categories. | Patient | User selection, query, upload, status change, or request payload as applicable. | Stores selected non-diagnostic categories with complaint entry. | Shows selected tags as organization aids. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Tags help structure doctor preparation view. |
...
- Product surface: Patient-side complaints preparation block.
- Upstream step: Patient opens complaints block before the visit.
- Downstream step: Complaints are shown as patient-provided context for doctor preparation.
...
- Internal storage or mocked service boundary for patient-entered complaint text and non-diagnostic tags.
...
2. System validates access and loads the minimum required data for Теги / категории жалоб.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: choose complaint tags/categories.
...
### Empty
- Show a clear empty state when no relevant patient-entered complaint text and non-diagnostic tags exists; include the next safe action if available.
...
- Description: Load only data the current actor is allowed to access for Теги / категории жалоб.
...
- Description: Support the core behavior where the user can choose complaint tags/categories.
...
- Description: Record sensitive actions and product funnel events relevant to Теги / категории жалоб.
...
|-------|---------|------------|----------------|
| complaint_tags_categories_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| complaint_tags_categories_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| complaint_tags_categories_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: ComplaintEntry, ComplaintTag, PreparationCase.
...
- Suggested path: product/features/017-complaint-tags-categories.md
- File role: Claude Code-ready feature specification for implementation planning and QA alignment.
- Related specs: Other specs in the same epic `Жалобы` and adjacent source-order preparation features.
