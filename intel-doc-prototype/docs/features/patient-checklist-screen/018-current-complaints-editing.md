...
- Source feature name, if different: Current complaints editing
- Feature layer / epic / product area: Layer 1 / Жалобы / Supporting

## 2. Purpose
This feature / flow step lets the patient edit current complaints so that пациент уточняет жалобы перед визитом. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 18; Feature layer = Layer 1; Epic = Жалобы; Epic flow = Пациент уточняет жалобы перед визитом; Feature = Редактирование актуальных жалоб; Epic path priority = Supporting.
- Normalized feature name: Current complaints editing
...
| Primary action | User performs the main action: edit current complaints. | Patient | User selection, query, upload, status change, or request payload as applicable. | Validates and saves patient-entered complaint text and tags. | Updates timestamps and visible current version. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Doctor sees updated patient-provided context if access exists. |
...
2. System validates access and loads the minimum required data for Редактирование актуальных жалоб.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: edit current complaints.
...
- Description: Load only data the current actor is allowed to access for Редактирование актуальных жалоб.
...
- Description: Support the core behavior where the user can edit current complaints.
...
|-------|---------|------------|----------------|
| current_complaints_editing_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| current_complaints_editing_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| current_complaints_editing_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/018-current-complaints-editing.md
