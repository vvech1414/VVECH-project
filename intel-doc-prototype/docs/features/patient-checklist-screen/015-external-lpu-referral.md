...
- Source feature name, if different: External LPU referral document
- Feature layer / epic / product area: Layer 1 / Подготовка документов / Supporting

## 2. Purpose
This feature lets the patient open external referral document item so that пациент прикладывает внешний медицинский документ. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 15; Feature layer = Layer 1; Epic = Подготовка документов; Epic flow = Пациент прикладывает внешний медицинский документ; Feature = Направление от другого ЛПУ; Epic path priority = Supporting.
- Normalized feature name: External LPU referral document
...
| Primary action | User performs the main action: open external referral document item. | Patient | User selection, query, upload, status change, or request payload as applicable. | Stores or displays referral original and basic metadata. | Marks item attached when original is available. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Referral is available to permitted doctor review. |
...
2. System validates access and loads the minimum required data for Направление от другого ЛПУ.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open external referral document item.
...
- Description: Load only data the current actor is allowed to access for Направление от другого ЛПУ.
...
- Description: Support the core behavior where the user can open external referral document item.
...
|-------|---------|------------|----------------|
| external_lpu_referral_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| external_lpu_referral_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| external_lpu_referral_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/015-external-lpu-referral.md
