...
- Source feature name, if different: New analyses list
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This screen section/block lets the patient open the new analyses plan so that пациент открывает план → видит, какие новые анализы нужно сдать. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 6; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Пациент открывает план → видит, какие новые анализы нужно сдать; Feature = Список новых анализов; Epic path priority = Core.
- Normalized feature name: New analyses list
...
| Primary action | User performs the main action: open the new analyses plan. | Patient | User selection, query, upload, status change, or request payload as applicable. | Loads plan items available to the patient. | Shows required analyses, status, and upload affordances. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient selects an item or uploads a result. |
...
2. System validates access and loads the minimum required data for Список новых анализов.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open the new analyses plan.
...
- Description: Load only data the current actor is allowed to access for Список новых анализов.
...
- Description: Support the core behavior where the user can open the new analyses plan.
...
|-------|---------|------------|----------------|
| new_analyses_list_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| new_analyses_list_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| new_analyses_list_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/006-new-analyses-list.md
