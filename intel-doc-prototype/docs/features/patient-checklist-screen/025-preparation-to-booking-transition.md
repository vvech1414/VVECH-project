...
- Source feature name, if different: Preparation to booking transition
- Feature layer / epic / product area: Layer 2 / Запись к основному врачу / Core

## 2. Purpose
This feature / flow step lets the patient complete enough preparation or choose to book so that пациент понимает, что запись является финальным шагом после подготовки. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 25; Feature layer = Layer 2; Epic = Запись к основному врачу; Epic flow = Пациент понимает, что запись является финальным шагом после подготовки; Feature = Переход из подготовки в запись; Epic path priority = Core.
- Normalized feature name: Preparation to booking transition
...
- Expected implementation target: Patient-side main doctor booking flow.
...
- Access control expectations: Patient access must be scoped to the owning patient/preparation case; Clinic staff or main doctor where explicitly supported by pilot workflow access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
- Auditability expectations: Appointment request creation and any sharing of preparation context are audit-sensitive.
...
- Original vs structured data handling, if relevant: Preparation data remains separate from booking request metadata.
...
| Primary action | User performs the main action: complete enough preparation or choose to book. | Patient | User selection, query, upload, status change, or request payload as applicable. | Moves patient from preparation checklist to main booking entry. | Carries preparation case context without exposing originals unnecessarily. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient begins booking flow. |
...
- Product surface: Patient-side main doctor booking flow.
- Upstream step: Patient has progressed through preparation and has no confirmed main appointment or chooses to book it.
- Downstream step: Patient reaches the booking request/confirmation state for the main doctor.
...
- Internal storage or mocked service boundary for main doctor booking state and transition from preparation.
...
2. System validates access and loads the minimum required data for Переход из подготовки в запись.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: complete enough preparation or choose to book.
...
### Empty
- Show a clear empty state when no relevant main doctor booking state and transition from preparation exists; include the next safe action if available.
...
- Description: Load only data the current actor is allowed to access for Переход из подготовки в запись.
...
- Description: Support the core behavior where the user can complete enough preparation or choose to book.
...
|-------|---------|------------|----------------|
| preparation_to_booking_transition_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| preparation_to_booking_transition_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| preparation_to_booking_transition_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: PreparationCase, MainAppointmentState, AppointmentRequest.
...
- Suggested path: product/features/025-preparation-to-booking-transition.md
- File role: Claude Code-ready feature specification for implementation planning and QA alignment.
- Related specs: Other specs in the same epic `Запись к основному врачу` and adjacent source-order preparation features.
