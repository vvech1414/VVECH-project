...
- Source feature name, if different: Old analyses period filter
- Feature layer / epic / product area: Layer 1 / Старые анализы / Supporting

## 2. Purpose
This feature / flow step lets the patient choose a date period filter so that пациент быстро находит нужные анализы для подготовки. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 3; Feature layer = Layer 1; Epic = Старые анализы; Epic flow = Пациент быстро находит нужные анализы для подготовки; Feature = Фильтрация старых анализов по периоду; Epic path priority = Supporting.
- Normalized feature name: Old analyses period filter
...
| Primary action | User performs the main action: choose a date period filter. | Patient | User selection, query, upload, status change, or request payload as applicable. | Filters loaded analysis records by result date with upload date fallback. | Shows matching records and active filter state. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Lets patient clear or change the period. |
...
2. System validates access and loads the minimum required data for Фильтрация старых анализов по периоду.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: choose a date period filter.
...
- Description: Load only data the current actor is allowed to access for Фильтрация старых анализов по периоду.
...
- Description: Support the core behavior where the user can choose a date period filter.
...
|-------|---------|------------|----------------|
| old_analyses_period_filter_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| old_analyses_period_filter_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| old_analyses_period_filter_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/003-old-analyses-period-filter.md
