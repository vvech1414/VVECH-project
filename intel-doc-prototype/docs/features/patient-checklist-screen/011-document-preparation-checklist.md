...
- Source feature name, if different: Document preparation checklist
- Feature layer / epic / product area: Layer 1 / Подготовка документов / Core

## 2. Purpose
This screen section/block lets the patient open document preparation so that пациент открывает раздел документов → понимает, что нужно подготовить до приёма. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 11; Feature layer = Layer 1; Epic = Подготовка документов; Epic flow = Пациент открывает раздел документов → понимает, что нужно подготовить до приёма; Feature = Чеклист документов для подготовки; Epic path priority = Core.
- Normalized feature name: Document preparation checklist
...
- Expected implementation target: Patient-side document preparation area.
...
- Access control expectations: Patient access must be scoped to the owning patient/preparation case; Doctor or admin with explicit access to the preparation case access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
- Auditability expectations: Document upload, deletion/replacement, doctor/admin view, and access changes are audit-sensitive.
...
- Original vs structured data handling, if relevant: Original uploaded document is preserved separately from structured/extracted metadata.
...
| Primary action | User performs the main action: open document preparation. | Patient | User selection, query, upload, status change, or request payload as applicable. | Loads required/supporting document checklist items. | Shows what is missing, ready, or optional. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient uploads or reviews each item. |
...
- Product surface: Patient-side document preparation area.
- Upstream step: Patient opens document preparation before an appointment or preparation case.
- Downstream step: Prepared documents are available for patient review and permitted doctor preparation.
...
- Internal storage or mocked service boundary for required and supporting documents for visit preparation.
...
2. System validates access and loads the minimum required data for Чеклист документов для подготовки.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open document preparation.
...
### Empty
- Show a clear empty state when no relevant required and supporting documents for visit preparation exists; include the next safe action if available.
...
- Description: Load only data the current actor is allowed to access for Чеклист документов для подготовки.
...
- Description: Support the core behavior where the user can open document preparation.
...
|-------|---------|------------|----------------|
| document_preparation_checklist_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| document_preparation_checklist_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| document_preparation_checklist_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: DocumentChecklistItem, OriginalDocument, StructuredExtraction, DocumentReadinessStatus.
...
- Suggested path: product/features/011-document-preparation-checklist.md
- File role: Claude Code-ready feature specification for implementation planning and QA alignment.
- Related specs: Other specs in the same epic `Подготовка документов` and adjacent source-order preparation features.
