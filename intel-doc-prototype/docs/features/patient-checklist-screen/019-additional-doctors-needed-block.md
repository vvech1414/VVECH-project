# Блок “какие ещё врачи могут понадобиться”

## 1. Title
- Feature name: Блок “какие ещё врачи могут понадобиться”
- Source feature name, if different: Additional doctors needed block
- Feature layer / epic / product area: Layer 2 / Запись к другим врачам для подготовки / Core

## 2. Purpose
This screen section/block lets the patient open additional doctors preparation block so that пациент понимает, что до основного врача могут понадобиться дополнительные консультации. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 19; Feature layer = Layer 2; Epic = Запись к другим врачам для подготовки; Epic flow = Пациент понимает, что до основного врача могут понадобиться дополнительные консультации; Feature = Блок “какие ещё врачи могут понадобиться”; Epic path priority = Core.
- Normalized feature name: Additional doctors needed block
...
- Expected implementation target: Patient-side preparation scheduling guidance.
...
- Access control expectations: Patient access must be scoped to the owning patient/preparation case; Clinic staff or doctor where explicitly supported by pilot workflow access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
- Auditability expectations: Appointment request creation and access to preparation context are audit-sensitive.
...
- Original vs structured data handling, if relevant: Any listed specialists are structured workflow options, not diagnostic conclusions.
...
| Primary action | User performs the main action: open additional doctors preparation block. | Patient | User selection, query, upload, status change, or request payload as applicable. | Shows predefined or clinician-configured specialist options where available. | Explains preparation relationship without medical certainty language. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient reviews options or continues. |
...
- Product surface: Patient-side preparation scheduling guidance.
- Upstream step: Patient reviews preparation steps before the main appointment.
- Downstream step: Patient selects a related specialist, requests booking, or returns to the preparation checklist.
...
- Internal storage or mocked service boundary for possible supporting specialist visits for preparation.
...
2. System validates access and loads the minimum required data for Блок “какие ещё врачи могут понадобиться”.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open additional doctors preparation block.
...
### Empty
- Show a clear empty state when no relevant possible supporting specialist visits for preparation exists; include the next safe action if available.
...
- Description: Load only data the current actor is allowed to access for Блок “какие ещё врачи могут понадобиться”.
...
- Description: Support the core behavior where the user can open additional doctors preparation block.
...
- Description: Record sensitive actions and product funnel events relevant to Блок “какие ещё врачи могут понадобиться”.
...
|-------|---------|------------|----------------|
| additional_doctors_needed_block_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| additional_doctors_needed_block_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| additional_doctors_needed_block_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: PreparationRecommendationBlock, SpecialistOption, AppointmentRequest, PreparationVisitLink.
...
- Suggested path: product/features/019-additional-doctors-needed-block.md
- File role: Claude Code-ready feature specification for implementation planning and QA alignment.
- Related specs: Other specs in the same epic `Запись к другим врачам для подготовки` and adjacent source-order preparation features.
