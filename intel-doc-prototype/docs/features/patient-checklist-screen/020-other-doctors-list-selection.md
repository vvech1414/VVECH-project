...
- Source feature name, if different: Other doctors list and selection
- Feature layer / epic / product area: Layer 2 / Запись к другим врачам для подготовки / Core

## 2. Purpose
This screen section/block lets the patient select a related specialist option so that пациент выбирает нужного смежного специалиста. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 20; Feature layer = Layer 2; Epic = Запись к другим врачам для подготовки; Epic flow = Пациент выбирает нужного смежного специалиста; Feature = Список / выбор других врачей; Epic path priority = Core.
- Normalized feature name: Other doctors list and selection
...
| Primary action | User performs the main action: select a related specialist option. | Patient | User selection, query, upload, status change, or request payload as applicable. | Records selected specialist within preparation flow. | Highlights selected option and enables next step. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient can request booking or change selection. |
...
2. System validates access and loads the minimum required data for Список / выбор других врачей.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: select a related specialist option.
...
- Description: Load only data the current actor is allowed to access for Список / выбор других врачей.
...
- Description: Support the core behavior where the user can select a related specialist option.
...
|-------|---------|------------|----------------|
| other_doctors_list_selection_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| other_doctors_list_selection_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| other_doctors_list_selection_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/020-other-doctors-list-selection.md
