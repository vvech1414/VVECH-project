...
- Source feature name, if different: Old analyses list
- Feature layer / epic / product area: Layer 1 / Старые анализы / Core

## 2. Purpose
This screen section/block lets the patient open the old analyses history so that пациент открывает историю анализов → просматривает ранее загруженные результаты → использует их для подготовки к основному приёму. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 1; Feature layer = Layer 1; Epic = Старые анализы; Epic flow = Пациент открывает историю анализов → просматривает ранее загруженные результаты → использует их для подготовки к основному приёму; Feature = Список старых анализов; Epic path priority = Core.
- Normalized feature name: Old analyses list
...
| Primary action | User performs the main action: open the old analyses history. | Patient | User selection, query, upload, status change, or request payload as applicable. | Loads patient-owned analysis records in stable newest-first order. | Shows a scannable list with metadata and status. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Opens the selected old analysis card. |
...
2. System validates access and loads the minimum required data for Список старых анализов.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open the old analyses history.
...
- Description: Load only data the current actor is allowed to access for Список старых анализов.
...
- Description: Support the core behavior where the user can open the old analyses history.
...
|-------|---------|------------|----------------|
| old_analyses_list_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| old_analyses_list_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| old_analyses_list_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/001-old-analyses-list.md
