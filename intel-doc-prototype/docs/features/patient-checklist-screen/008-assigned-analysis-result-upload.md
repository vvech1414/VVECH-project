...
- Source feature name, if different: Assigned analysis result upload
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This feature / flow step lets the patient start upload from an assigned analysis item so that пациент завершает пункт плана → добавляет результат анализа. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 8; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Пациент завершает пункт плана → добавляет результат анализа; Feature = Загрузка результата по назначенному анализу; Epic path priority = Core.
- Normalized feature name: Assigned analysis result upload
...
- Access control expectations: Patient access must be scoped to the owning patient/preparation case; Doctor who created or can view the plan with explicit access access requires explicit grant, appointment-scoped access, or another approved policy before sensitive data is rendered.
...
| Primary action | User performs the main action: start upload from an assigned analysis item. | Patient | User selection, query, upload, status change, or request payload as applicable. | Stores the original result document and provisional metadata. | Associates upload intent with the plan item. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Moves to linking/quality state. |
...
2. System validates access and loads the minimum required data for Загрузка результата по назначенному анализу.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: start upload from an assigned analysis item.
...
- Description: Load only data the current actor is allowed to access for Загрузка результата по назначенному анализу.
...
- Description: Support the core behavior where the user can start upload from an assigned analysis item.
...
|-------|---------|------------|----------------|
| assigned_analysis_result_upload_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| assigned_analysis_result_upload_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| assigned_analysis_result_upload_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/008-assigned-analysis-result-upload.md
