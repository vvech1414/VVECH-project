...
- Source feature name, if different: Result linking to plan item
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This feature / flow step lets the patient finish or confirm upload for a plan item so that система связывает загруженный результат с соответствующим пунктом плана. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 9; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Система связывает загруженный результат с соответствующим пунктом плана; Feature = Привязка результата к пункту плана; Epic path priority = Core.
- Normalized feature name: Result linking to plan item
...
| Primary action | User performs the main action: finish or confirm upload for a plan item. | Patient | User selection, query, upload, status change, or request payload as applicable. | Links the uploaded result to the selected plan item after validation. | Marks the item as having a result attached. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Result becomes available for dossier aggregation. |
...
2. System validates access and loads the minimum required data for Привязка результата к пункту плана.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: finish or confirm upload for a plan item.
...
- Description: Load only data the current actor is allowed to access for Привязка результата к пункту плана.
...
- Description: Support the core behavior where the user can finish or confirm upload for a plan item.
...
|-------|---------|------------|----------------|
| result_to_plan_item_linking_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| result_to_plan_item_linking_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| result_to_plan_item_linking_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/009-result-to-plan-item-linking.md
