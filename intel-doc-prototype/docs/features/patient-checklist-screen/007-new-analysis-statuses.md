...
- Source feature name, if different: New analysis statuses
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This supporting system behavior lets the patient view or update completion status so that пациент отслеживает выполнение плана. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 7; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Пациент отслеживает выполнение плана; Feature = Статусы новых анализов; Epic path priority = Core.
- Normalized feature name: New analysis statuses
...
| Primary action | User performs the main action: view or update completion status. | Patient | User selection, query, upload, status change, or request payload as applicable. | Computes each item state from plan and upload/link data. | Shows not started, uploaded, linked, or needs attention states. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient continues completion or doctor reviews readiness. |
...
2. System validates access and loads the minimum required data for Статусы новых анализов.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: view or update completion status.
...
- Description: Load only data the current actor is allowed to access for Статусы новых анализов.
...
- Description: Support the core behavior where the user can view or update completion status.
...
|-------|---------|------------|----------------|
| new_analysis_statuses_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| new_analysis_statuses_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| new_analysis_statuses_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/007-new-analysis-statuses.md
