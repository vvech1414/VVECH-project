...
- Source feature name, if different: Main appointment absent state
- Feature layer / epic / product area: Layer 2 / Запись к основному врачу / Core

## 2. Purpose
This screen section/block lets the patient open main booking area with no main appointment so that пациент видит, что основной записи ещё нет. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 23; Feature layer = Layer 2; Epic = Запись к основному врачу; Epic flow = Пациент видит, что основной записи ещё нет; Feature = Состояние “основная запись отсутствует”; Epic path priority = Core.
- Normalized feature name: Main appointment absent state
...
| Primary action | User performs the main action: open main booking area with no main appointment. | Patient | User selection, query, upload, status change, or request payload as applicable. | Detects absence of confirmed main appointment. | Shows an empty/blocked state with booking cta. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient starts main doctor booking. |
...
2. System validates access and loads the minimum required data for Состояние “основная запись отсутствует”.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: open main booking area with no main appointment.
...
- Description: Load only data the current actor is allowed to access for Состояние “основная запись отсутствует”.
...
- Description: Support the core behavior where the user can open main booking area with no main appointment.
...
|-------|---------|------------|----------------|
| main_appointment_absent_state_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| main_appointment_absent_state_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| main_appointment_absent_state_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/023-main-appointment-absent-state.md
