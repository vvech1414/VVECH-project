...
- Source feature name, if different: Book main doctor
- Feature layer / epic / product area: Layer 2 / Запись к основному врачу / Core

## 2. Purpose
This feature / flow step lets the patient start main doctor booking so that пациент инициирует запись к основному специалисту. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 24; Feature layer = Layer 2; Epic = Запись к основному врачу; Epic flow = Пациент инициирует запись к основному специалисту; Feature = Запись к основному врачу; Epic path priority = Core.
- Normalized feature name: Book main doctor
...
| Primary action | User performs the main action: start main doctor booking. | Patient | User selection, query, upload, status change, or request payload as applicable. | Creates a booking request or routes to approved scheduling surface. | Shows pending/confirmed/error state. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient receives booking status. |
...
2. System validates access and loads the minimum required data for Запись к основному врачу.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: start main doctor booking.
...
- Description: Load only data the current actor is allowed to access for Запись к основному врачу.
...
- Description: Support the core behavior where the user can start main doctor booking.
...
|-------|---------|------------|----------------|
| book_main_doctor_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| book_main_doctor_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| book_main_doctor_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/024-book-main-doctor.md
