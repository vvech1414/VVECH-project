...
- Source feature name, if different: Document upload
- Feature layer / epic / product area: Layer 1 / Подготовка документов / Core

## 2. Purpose
This feature / flow step lets the patient choose a checklist item or generic upload action so that пациент добавляет обязательные документы. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 12; Feature layer = Layer 1; Epic = Подготовка документов; Epic flow = Пациент добавляет обязательные документы; Feature = Загрузка документов; Epic path priority = Core.
- Normalized feature name: Document upload
...
| Primary action | User performs the main action: choose a checklist item or generic upload action. | Patient | User selection, query, upload, status change, or request payload as applicable. | Stores original document and optional structured metadata. | Shows upload quality/status feedback. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Document appears in checklist/readiness state. |
...
2. System validates access and loads the minimum required data for Загрузка документов.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: choose a checklist item or generic upload action.
...
- Description: Load only data the current actor is allowed to access for Загрузка документов.
...
- Description: Support the core behavior where the user can choose a checklist item or generic upload action.
...
|-------|---------|------------|----------------|
| document_upload_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| document_upload_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| document_upload_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/012-document-upload.md
