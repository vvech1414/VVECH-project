...
- Source feature name, if different: Book another doctor
- Feature layer / epic / product area: Layer 2 / Запись к другим врачам для подготовки / Core

## 2. Purpose
This feature / flow step lets the patient request booking for selected related specialist so that пациент инициирует дополнительную консультацию. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 21; Feature layer = Layer 2; Epic = Запись к другим врачам для подготовки; Epic flow = Пациент инициирует дополнительную консультацию; Feature = Запись к другому врачу; Epic path priority = Core.
- Normalized feature name: Book another doctor
...
| Primary action | User performs the main action: request booking for selected related specialist. | Patient | User selection, query, upload, status change, or request payload as applicable. | Creates a minimal appointment request or handoff record. | Shows request/confirmation state based on pilot scheduling capability. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Patient returns to preparation after booking step. |
...
2. System validates access and loads the minimum required data for Запись к другому врачу.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: request booking for selected related specialist.
...
- Description: Load only data the current actor is allowed to access for Запись к другому врачу.
...
- Description: Support the core behavior where the user can request booking for selected related specialist.
...
|-------|---------|------------|----------------|
| book_other_doctor_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| book_other_doctor_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| book_other_doctor_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/021-book-other-doctor.md
