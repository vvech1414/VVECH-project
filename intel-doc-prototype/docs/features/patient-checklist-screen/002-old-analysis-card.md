...
- Source feature name, if different: Old analysis card
- Feature layer / epic / product area: Layer 1 / Старые анализы / Core

## 2. Purpose
This screen section/block lets the patient select an old analysis from history so that пациент открывает конкретный анализ → видит его содержимое и контекст. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 2; Feature layer = Layer 1; Epic = Старые анализы; Epic flow = Пациент открывает конкретный анализ → видит его содержимое и контекст; Feature = Карточка старого анализа; Epic path priority = Core.
- Normalized feature name: Old analysis card
...
| Primary action | User performs the main action: select an old analysis from history. | Patient | User selection, query, upload, status change, or request payload as applicable. | Loads the selected record after ownership/access check. | Shows original document access, extracted metadata, and context separately. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Returns to list or opens original preview. |
...
2. System validates access and loads the minimum required data for Карточка старого анализа.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: select an old analysis from history.
...
- Description: Load only data the current actor is allowed to access for Карточка старого анализа.
...
- Description: Support the core behavior where the user can select an old analysis from history.
...
|-------|---------|------------|----------------|
| old_analysis_card_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| old_analysis_card_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| old_analysis_card_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/002-old-analysis-card.md
