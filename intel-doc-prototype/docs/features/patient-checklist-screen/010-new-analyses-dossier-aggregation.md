...
- Source feature name, if different: New analyses dossier aggregation
- Feature layer / epic / product area: Layer 1 / План новых анализов / Core

## 2. Purpose
This feature / flow step lets the patient complete a new analysis result flow so that новые результаты попадают в единый контур подготовки к визиту. It exists to support the IntelDoc preparation workflow with minimal integration assumptions and explicit handling of sensitive patient data.

## 3. Source normalization notes
- Source row/block: row 10; Feature layer = Layer 1; Epic = План новых анализов; Epic flow = Новые результаты попадают в единый контур подготовки к визиту; Feature = Агрегация новых анализов в досье; Epic path priority = Core.
- Normalized feature name: New analyses dossier aggregation
...
| Primary action | User performs the main action: complete a new analysis result flow. | Patient | User selection, query, upload, status change, or request payload as applicable. | Adds linked results into the preparation dossier index. | Shows dossier inclusion state while keeping originals separate. |
| Follow-through | User continues the preparation journey. | Patient | Current feature state and any saved object references. | Persist allowed changes and keep originals separate from structured data. | Doctor or patient sees the result in preparation context. |
...
2. System validates access and loads the minimum required data for Агрегация новых анализов в досье.
3. System displays the default state with clear source/status information and available actions.
4. Patient completes the primary action: complete a new analysis result flow.
...
- Description: Load only data the current actor is allowed to access for Агрегация новых анализов в досье.
...
- Description: Support the core behavior where the user can complete a new analysis result flow.
...
|-------|---------|------------|----------------|
| new_analyses_dossier_aggregation_viewed | User opens the feature surface after access validation. | feature_id, epic, layer, priority, actor_type, has_existing_data | Feature reach and empty/error rates are measurable. |
| new_analyses_dossier_aggregation_primary_action | User completes the primary action for the feature. | feature_id, action_type, result_state, priority, preparation_case_id_hash | Users can progress through the preparation flow. |
| new_analyses_dossier_aggregation_blocked | System blocks access or disables the primary action. | feature_id, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested path: product/features/010-new-analyses-dossier-aggregation.md
