...
- **Шаг:** 2
- **Экран:** Consent
- **Действия пользователя:** Отмечает обязательное согласие; опционально — согласие на reminder; жмёт «Принять и продолжить»
- **Данные системы:** consentVersion, timestamp, optInReminder
- **Happy path:** Видит компактный текст согласия и две галочки; принимает и идёт дальше
- **JTBD:** Подтвердить согласие без чтения юридической стены
...
- **N1** Consent v1 — Версионируемый, с timestamp, с обязательным согласием — flow согласий пациента

## 5. Состояния экрана (state coverage)

- **default** — Базовый вид экрана «Consent» с заполненными mock-данными. ★ обязательно для демо
...
Сырые поля из источника: `consentVersion, timestamp, optInReminder`
...
- «Я даю согласие на обработку данных для подготовки к приёму»
...
- [ ] Обязательное согласие нельзя пропустить
- [ ] версия и timestamp фиксируются (N1)
