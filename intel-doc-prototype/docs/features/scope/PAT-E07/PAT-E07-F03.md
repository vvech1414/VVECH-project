...
- **Шаг:** 3
- **Экран:** OCR status chips
- **Действия пользователя:** Видит чипы статуса распознавания на карточках
- **Данные системы:** ocrStatus: structured|originalOnly|inReview
- **Happy path:** Видит chip «Распознано» / «Только оригинал»
- **JTBD:** Понять какие анализы готовы для врача
...
- **default** — Базовый вид экрана «OCR status chips» с заполненными mock-данными. ★ обязательно для демо
...
Сырые поля из источника: `ocrStatus: structured|originalOnly|inReview`
...
- [ ] Chip явно показывает: structured / original-only
...
- **Кандидаты файлов:** `src/routes/patient/UploadFlow.tsx`, `src/routes/patient/History.tsx`
