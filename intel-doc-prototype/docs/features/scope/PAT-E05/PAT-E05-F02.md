...
# PAT-E05-F02 — Поворот / кроп / контраст
...
- **Шаг:** 2
- **Экран:** Поворот / кроп / контраст
- **Действия пользователя:** Корректирует ориентацию и кадрирование
- **Данные системы:** transform: {rotate, crop, contrast}
- **Happy path:** Применяет преобразования и видит результат
- **JTBD:** Привести документ к читаемому виду
...
- **default** — Базовый вид экрана «Поворот / кроп / контраст» с заполненными mock-данными. ★ обязательно для демо
...
Сырые поля из источника: `transform: {rotate, crop, contrast}`

```ts
{
  transform: { rotate: 0, crop: null, contrast: 1.0 },
}
```
