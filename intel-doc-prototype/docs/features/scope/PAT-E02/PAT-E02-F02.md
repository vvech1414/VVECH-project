...
- **Шаг:** 2
- **Экран:** Prep banner
- **Действия пользователя:** Видит карточку статуса подготовки и primary CTA
- **Данные системы:** prepStatus, nextAppointmentDate, openTasksCount
- **Happy path:** Видит «Подготовьтесь к приёму», прогресс и одну кнопку
- **JTBD:** Понять статус подготовки за 5 секунд
...
- **default** — Базовый вид экрана «Prep banner» с заполненными mock-данными. ★ обязательно для демо
...
Сырые поля из источника: `prepStatus, nextAppointmentDate, openTasksCount`

```ts
{
  prepStatus: "in-progress",
  nextAppointmentDate: "2026-05-08T10:00:00Z",
  openTasksCount: 2,
}
```
...
- [ ] Один баннер показывает статус и одну primary CTA («Начать подготовку» или «Продолжить подготовку»)
...
- **Кандидаты файлов:** `src/routes/patient/Home.tsx`, `src/routes/patient/Checklist.tsx`
