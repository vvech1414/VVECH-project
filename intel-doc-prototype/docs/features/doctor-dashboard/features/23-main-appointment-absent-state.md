# Состояние "основная запись отсутствует"

## 1. Title
- Feature name: Состояние "основная запись отсутствует"
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Запись к основному врачу / Core

## 2. Purpose
This screen state supports the patient dashboard preparation journey by letting the patient views the dashboard when no main appointment is recorded. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 23; Epic = Запись к основному врачу; Epic Flow = Пациент видит, что основной записи еще нет.; Feature = Состояние "основная запись отсутствует"; Happy path = Core; Priority = Must-have.
- Normalized feature name: Состояние "основная запись отсутствует"
...
- Expected implementation target: Patient dashboard / main doctor booking.
...
| Primary behavior | Patient views the dashboard when no main appointment is recorded. | Patient | User action and relevant object ids or form values. | shows a non-revealing absent state and a permitted next action for main doctor booking. | patient understands that main appointment is not yet set. |
...
- Product surface: Patient dashboard / main doctor booking.
- Upstream step: Patient opens dashboard before a main doctor appointment exists in IntelDoc.
- Downstream step: Patient initiates main doctor booking or continues preparation.
...
## 9. Scope
### In scope
- Implement the source feature intent for Состояние "основная запись отсутствует" inside the patient dashboard preparation journey.
...
### Happy path
1. Patient opens the relevant dashboard area for Запись к основному врачу.
2. System validates access to the preparation case and loads only data needed for Состояние "основная запись отсутствует".
3. System shows the default state with source/status context and permitted actions.
4. Patient views the dashboard when no main appointment is recorded.
5. System shows a non-revealing absent state and a permitted next action for main doctor booking and records required analytics/audit events.
6. Patient sees the next state: patient understands that main appointment is not yet set.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Состояние "основная запись отсутствует".
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Состояние "основная запись отсутствует".
...
- Description: Load Состояние "основная запись отсутствует" only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient views the dashboard when no main appointment is recorded.
...
- Description: Record sensitive actions and product funnel events relevant to Состояние "основная запись отсутствует".
...
- The feature follows this spec and preserves source row 23 intent.
...
|-------|---------|------------|----------------|
| main_appointment_absent_state_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| main_appointment_absent_state_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| main_appointment_absent_state_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |

## 16. Suggested implementation notes for Claude Code
- Suggested components/modules: Feature-specific dashboard block or flow component for Запись к основному врачу; shared status, empty/error, permission, audit, and analytics helpers if present.
- Suggested data objects: MainDoctorAppointment, BookingStatus, PreparationCase, DashboardState.
...
- Suggested path: product/features/doctor-dashboard/23-main-appointment-absent-state.md
