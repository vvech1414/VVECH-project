# Список новых анализов

## 1. Title
- Feature name: Список новых анализов
...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient views the list of new analyses to complete. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 6; Epic = План новых анализов; Epic Flow = Пациент открывает план и видит, какие новые анализы нужно сдать.; Feature = Список новых анализов; Happy path = Core; Priority = Must-have.
- Normalized feature name: Список новых анализов
...
| Primary behavior | Patient views the list of new analyses to complete. | Patient | User action and relevant object ids or form values. | renders plan items with status, due/preparation context if present, and no external lab assumptions. | new analyses are visible as actionable preparation items. |
...
- Upstream step: A doctor-created or approved analysis plan exists for the preparation case.
- Downstream step: Patient opens an item, uploads a result, or tracks status.
...
## 9. Scope
### In scope
- Implement the source feature intent for Список новых анализов inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Список новых анализов.
3. System shows the default state with source/status context and permitted actions.
4. Patient views the list of new analyses to complete.
5. System renders plan items with status, due/preparation context if present, and no external lab assumptions and records required analytics/audit events.
6. Patient sees the next state: new analyses are visible as actionable preparation items.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Список новых анализов.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Список новых анализов.
...
- Description: Load Список новых анализов only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient views the list of new analyses to complete.
...
- Description: Record sensitive actions and product funnel events relevant to Список новых анализов.
...
- The feature follows this spec and preserves source row 6 intent.
...
|-------|---------|------------|----------------|
| new_analyses_list_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| new_analyses_list_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| new_analyses_list_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisPlan, AnalysisPlanItem, AnalysisStatus, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/06-new-analyses-list.md
