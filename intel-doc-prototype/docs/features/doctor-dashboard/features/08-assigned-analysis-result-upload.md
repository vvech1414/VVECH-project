# Загрузка результата по назначенному анализу

## 1. Title
- Feature name: Загрузка результата по назначенному анализу
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / План новых анализов / Core

## 2. Purpose
This flow step supports the patient dashboard preparation journey by letting the patient uploads a result for an assigned analysis plan item. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 8; Epic = План новых анализов; Epic Flow = Пациент завершает пункт плана и добавляет результат анализа.; Feature = Загрузка результата по назначенному анализу; Happy path = Core; Priority = Must-have.
- Normalized feature name: Загрузка результата по назначенному анализу
...
- Assumptions made: MVP uses IntelDoc-owned preparation records, patient-controlled access, and internal/mocked service boundaries. External lab systems, LIS integrations, EHR import, and automatic result retrieval are future dependency / out of scope for MVP unless explicitly approved.
...
- Expected implementation target: Patient dashboard / new analyses plan item.
...
- Minimal integration assumptions: External lab systems, LIS integrations, EHR import, and automatic result retrieval are future dependency / out of scope for MVP unless explicitly approved.
...
| Primary behavior | Patient uploads a result for an assigned analysis plan item. | Patient | User action and relevant object ids or form values. | stores the original uploaded file and associates it with the selected plan item after validation. | result upload is available for review as an original document plus optional structured metadata. |
...
- Product surface: Patient dashboard / new analyses plan item.
- Upstream step: Patient opens an assigned analysis that allows result upload.
- Downstream step: System links the uploaded result to the plan item and updates status.
...
## 9. Scope
### In scope
- Implement the source feature intent for Загрузка результата по назначенному анализу inside the patient dashboard preparation journey.
...
### Happy path
1. Patient opens the relevant dashboard area for План новых анализов.
2. System validates access to the preparation case and loads only data needed for Загрузка результата по назначенному анализу.
3. System shows the default state with source/status context and permitted actions.
4. Patient uploads a result for an assigned analysis plan item.
5. System stores the original uploaded file and associates it with the selected plan item after validation and records required analytics/audit events.
6. Patient sees the next state: result upload is available for review as an original document plus optional structured metadata.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Загрузка результата по назначенному анализу.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Загрузка результата по назначенному анализу.
...
- Description: Load Загрузка результата по назначенному анализу only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient uploads a result for an assigned analysis plan item.
- Rules: Keep the behavior limited to the source row intent and current preparation context. External lab systems, LIS integrations, EHR import, and automatic result retrieval are future dependency / out of scope for MVP unless explicitly approved.
...
- Description: Record sensitive actions and product funnel events relevant to Загрузка результата по назначенному анализу.
...
- The feature follows this spec and preserves source row 8 intent.
...
|-------|---------|------------|----------------|
| assigned_analysis_result_upload_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| assigned_analysis_result_upload_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| assigned_analysis_result_upload_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |

## 16. Suggested implementation notes for Claude Code
- Suggested components/modules: Feature-specific dashboard block or flow component for План новых анализов; shared status, empty/error, permission, audit, and analytics helpers if present.
- Suggested data objects: AnalysisPlanItem, ResultUpload, OriginalAnalysisDocument, StructuredAnalysisMetadata.
...
- Suggested path: product/features/doctor-dashboard/08-assigned-analysis-result-upload.md
