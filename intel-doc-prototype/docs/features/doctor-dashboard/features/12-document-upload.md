...
## 1. Title
- Feature name: Загрузка документов
...
## 2. Purpose
This flow step supports the patient dashboard preparation journey by letting the patient uploads a preparation document. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 12; Epic = Подготовка документов; Epic Flow = Пациент добавляет обязательные документы.; Feature = Загрузка документов; Happy path = Core; Priority = Must-have.
- Normalized feature name: Загрузка документов
...
| Primary behavior | Patient uploads a preparation document. | Patient | User action and relevant object ids or form values. | validates, stores, and previews the original document while keeping any extraction separate. | document slot is filled or marked as upload failed with retry. |
...
- Upstream step: Patient selects a checklist slot or generic upload action.
- Downstream step: Document readiness status updates after successful upload.
...
## 9. Scope
### In scope
- Implement the source feature intent for Загрузка документов inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Загрузка документов.
3. System shows the default state with source/status context and permitted actions.
4. Patient uploads a preparation document.
5. System validates, stores, and previews the original document while keeping any extraction separate and records required analytics/audit events.
6. Patient sees the next state: document slot is filled or marked as upload failed with retry.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Загрузка документов.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Загрузка документов.
...
- Description: Load Загрузка документов only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient uploads a preparation document.
...
- Description: Record sensitive actions and product funnel events relevant to Загрузка документов.
...
- The feature follows this spec and preserves source row 12 intent.
...
|-------|---------|------------|----------------|
| document_upload_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| document_upload_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| document_upload_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: DocumentUpload, DocumentSlot, OriginalDocument, StructuredDocumentMetadata.
...
- Suggested path: product/features/doctor-dashboard/12-document-upload.md
