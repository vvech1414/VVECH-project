...
- Feature layer / epic / product area: Patient dashboard / preparation journey / Жалобы / Core

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient assigns tags or categories to self-reported complaints. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 17; Epic = Жалобы; Epic Flow = Пациент структурирует свои жалобы в удобной форме.; Feature = Теги / категории жалоб; Happy path = Core; Priority = Must-have.
- Normalized feature name: Теги / категории жалоб
...
| Primary behavior | Patient assigns tags or categories to self-reported complaints. | Patient | User action and relevant object ids or form values. | stores selected non-diagnostic categories for organization and doctor preparation. | complaints are grouped or labeled for easier review. |
...
- Upstream step: Patient has entered or is entering complaints.
- Downstream step: Patient reviews categorized complaints or edits them before the visit.
...
## 9. Scope
### In scope
- Implement the source feature intent for Теги / категории жалоб inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Теги / категории жалоб.
3. System shows the default state with source/status context and permitted actions.
4. Patient assigns tags or categories to self-reported complaints.
5. System stores selected non-diagnostic categories for organization and doctor preparation and records required analytics/audit events.
6. Patient sees the next state: complaints are grouped or labeled for easier review.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Теги / категории жалоб.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Теги / категории жалоб.
...
- Description: Load Теги / категории жалоб only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient assigns tags or categories to self-reported complaints.
...
- The feature follows this spec and preserves source row 17 intent.
...
|-------|---------|------------|----------------|
| complaint_tags_categories_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| complaint_tags_categories_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| complaint_tags_categories_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: ComplaintEntry, ComplaintCategory, CategorySelection, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/17-complaint-tags-categories.md
