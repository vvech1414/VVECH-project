# Чеклист документов для подготовки

## 1. Title
- Feature name: Чеклист документов для подготовки
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Подготовка документов / Core

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient opens the preparation document checklist. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 11; Epic = Подготовка документов; Epic Flow = Пациент открывает раздел документов и понимает, что нужно подготовить до приема.; Feature = Чеклист документов для подготовки; Happy path = Core; Priority = Must-have.
- Normalized feature name: Чеклист документов для подготовки
...
| Primary behavior | Patient opens the preparation document checklist. | Patient | User action and relevant object ids or form values. | shows required and optional document slots for the preparation case using approved checklist definitions. | patient sees which documents are missing, uploaded, or optional. |
...
- Upstream step: Patient opens the dashboard document section.
- Downstream step: Patient uploads documents or reviews document readiness status.
...
## 9. Scope
### In scope
- Implement the source feature intent for Чеклист документов для подготовки inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Чеклист документов для подготовки.
3. System shows the default state with source/status context and permitted actions.
4. Patient opens the preparation document checklist.
5. System shows required and optional document slots for the preparation case using approved checklist definitions and records required analytics/audit events.
6. Patient sees the next state: patient sees which documents are missing, uploaded, or optional.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Чеклист документов для подготовки.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Чеклист документов для подготовки.
...
- Description: Load Чеклист документов для подготовки only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient opens the preparation document checklist.
...
- Description: Record sensitive actions and product funnel events relevant to Чеклист документов для подготовки.
...
- The feature follows this spec and preserves source row 11 intent.
...
|-------|---------|------------|----------------|
| document_preparation_checklist_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| document_preparation_checklist_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| document_preparation_checklist_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: DocumentChecklist, DocumentSlot, OriginalDocument, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/11-document-preparation-checklist.md
