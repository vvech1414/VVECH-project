...
## 1. Title
- Feature name: Запись к другому врачу
...
## 2. Purpose
This flow step supports the patient dashboard preparation journey by letting the patient initiates a booking request for another doctor. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 21; Epic = Запись к другим врачам для подготовки; Epic Flow = Пациент инициирует дополнительную консультацию.; Feature = Запись к другому врачу; Happy path = Core; Priority = Must-have.
- Normalized feature name: Запись к другому врачу
...
- Expected implementation target: Patient dashboard / additional doctor booking.
...
| Primary behavior | Patient initiates a booking request for another doctor. | Patient | User action and relevant object ids or form values. | creates an internal booking request or partner handoff; direct external scheduling is future dependency. | request is created with pending/confirmed/failed pilot-safe status. |
...
- Product surface: Patient dashboard / additional doctor booking.
- Upstream step: Patient selected an additional doctor or specialty.
- Downstream step: Patient sees request status and relation to the main doctor preparation flow.
...
## 9. Scope
### In scope
- Implement the source feature intent for Запись к другому врачу inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Запись к другому врачу.
3. System shows the default state with source/status context and permitted actions.
4. Patient initiates a booking request for another doctor.
5. System creates an internal booking request or partner handoff; direct external scheduling is future dependency and records required analytics/audit events.
6. Patient sees the next state: request is created with pending/confirmed/failed pilot-safe status.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Запись к другому врачу.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Запись к другому врачу.
...
- Description: Load Запись к другому врачу only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient initiates a booking request for another doctor.
...
- Description: Record sensitive actions and product funnel events relevant to Запись к другому врачу.
...
- The feature follows this spec and preserves source row 21 intent.
...
|-------|---------|------------|----------------|
| book_other_doctor_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| book_other_doctor_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| book_other_doctor_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: BookingRequest, DoctorOption, PreparationCase, BookingStatus.
...
- Suggested path: product/features/doctor-dashboard/21-book-other-doctor.md
