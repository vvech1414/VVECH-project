...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient adds or views an OMS policy document for visit preparation. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 14; Epic = Подготовка документов; Epic Flow = Пациент хранит документ, который может понадобиться при визите.; Feature = ОМС; Happy path = Supporting; Priority = Should-have.
- Normalized feature name: ОМС
...
| Primary behavior | Patient adds or views an OMS policy document for visit preparation. | Patient | User action and relevant object ids or form values. | stores the original OMS document as a patient-controlled preparation document without insurance verification integration. | OMS document slot is available as uploaded, missing, or optional. |
...
- Upstream step: Patient opens the document checklist and sees an OMS-related slot if configured.
- Downstream step: Patient uploads/replaces the OMS document or continues without it if optional.
...
## 9. Scope
### In scope
- Implement the source feature intent for ОМС inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for ОМС.
3. System shows the default state with source/status context and permitted actions.
4. Patient adds or views an OMS policy document for visit preparation.
5. System stores the original OMS document as a patient-controlled preparation document without insurance verification integration and records required analytics/audit events.
6. Patient sees the next state: OMS document slot is available as uploaded, missing, or optional.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for ОМС.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for ОМС.
...
- Description: Load ОМС only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient adds or views an OMS policy document for visit preparation.
...
- Description: Record sensitive actions and product funnel events relevant to ОМС.
...
- The feature follows this spec and preserves source row 14 intent.
...
|-------|---------|------------|----------------|
| oms_document_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| oms_document_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| oms_document_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: DocumentSlot, OriginalDocument, OmsDocumentMetadata, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/14-oms-document.md
