# Список старых анализов

## 1. Title
- Feature name: Список старых анализов
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Старые анализы / Core

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient opens the old analyses history. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 1; Epic = Старые анализы; Epic Flow = Пациент открывает историю анализов и использует ранее загруженные результаты для подготовки к основному приему.; Feature = Список старых анализов; Happy path = Core; Priority = Must-have.
- Normalized feature name: Список старых анализов
...
| Primary behavior | Patient opens the old analyses history. | Patient | User action and relevant object ids or form values. | loads patient-owned prior analysis records in a stable, scannable order. | old analysis list is visible with source and status metadata. |
...
- Upstream step: Patient opens the IntelDoc dashboard before or during preparation for the main doctor visit.
- Downstream step: Patient opens an old analysis card or uses filter/search to find a relevant result.
...
## 9. Scope
### In scope
- Implement the source feature intent for Список старых анализов inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Список старых анализов.
3. System shows the default state with source/status context and permitted actions.
4. Patient opens the old analyses history.
5. System loads patient-owned prior analysis records in a stable, scannable order and records required analytics/audit events.
6. Patient sees the next state: old analysis list is visible with source and status metadata.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Список старых анализов.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Список старых анализов.
...
- Description: Load Список старых анализов only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient opens the old analyses history.
...
- Description: Record sensitive actions and product funnel events relevant to Список старых анализов.
...
- The feature follows this spec and preserves source row 1 intent.
...
|-------|---------|------------|----------------|
| old_analyses_list_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| old_analyses_list_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| old_analyses_list_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisRecord, OriginalAnalysisDocument, StructuredAnalysisMetadata, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/01-old-analyses-list.md
