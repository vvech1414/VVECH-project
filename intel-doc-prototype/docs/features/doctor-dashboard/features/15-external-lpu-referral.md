# Направление от другого ЛПУ

## 1. Title
- Feature name: Направление от другого ЛПУ
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Подготовка документов / Supporting

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient uploads or views a referral from another medical organization. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 15; Epic = Подготовка документов; Epic Flow = Пациент прикладывает внешний медицинский документ для подготовки к визиту.; Feature = Направление от другого ЛПУ; Happy path = Supporting; Priority = Should-have.
- Normalized feature name: Направление от другого ЛПУ
...
- Assumptions made: MVP uses IntelDoc-owned preparation records, patient-controlled access, and internal/mocked service boundaries. External document registries, insurance validation, LPU system integrations, and OCR automation beyond approved MVP boundaries are future dependency / out of scope for MVP unless explicitly approved.
...
- Expected implementation target: Patient dashboard / document preparation.
...
- Minimal integration assumptions: External document registries, insurance validation, LPU system integrations, and OCR automation beyond approved MVP boundaries are future dependency / out of scope for MVP unless explicitly approved.
...
- Original vs structured data handling, if relevant: Original uploaded files must remain distinct from structured/extracted metadata; structured fields are source-backed aids, not replacements for originals.
...
| Primary behavior | Patient uploads or views a referral from another medical organization. | Patient | User action and relevant object ids or form values. | stores the original referral and optional patient-entered metadata without connecting to external LPU systems. | external referral is available as a source document for preparation. |
...
- Product surface: Patient dashboard / document preparation.
- Upstream step: Patient has a referral document that may be useful for the main visit.
- Downstream step: Referral appears in the document checklist and preparation dossier where access is approved.
...
## 9. Scope
### In scope
- Implement the source feature intent for Направление от другого ЛПУ inside the patient dashboard preparation journey.
...
### Happy path
1. Patient opens the relevant dashboard area for Подготовка документов.
2. System validates access to the preparation case and loads only data needed for Направление от другого ЛПУ.
3. System shows the default state with source/status context and permitted actions.
4. Patient uploads or views a referral from another medical organization.
5. System stores the original referral and optional patient-entered metadata without connecting to external LPU systems and records required analytics/audit events.
6. Patient sees the next state: external referral is available as a source document for preparation.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Направление от другого ЛПУ.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Направление от другого ЛПУ.
...
- Description: Load Направление от другого ЛПУ only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient uploads or views a referral from another medical organization.
- Rules: Keep the behavior limited to the source row intent and current preparation context. External document registries, insurance validation, LPU system integrations, and OCR automation beyond approved MVP boundaries are future dependency / out of scope for MVP unless explicitly approved.
...
- Description: Record sensitive actions and product funnel events relevant to Направление от другого ЛПУ.
...
- The feature follows this spec and preserves source row 15 intent.
...
|-------|---------|------------|----------------|
| external_lpu_referral_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| external_lpu_referral_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| external_lpu_referral_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |

## 16. Suggested implementation notes for Claude Code
- Suggested components/modules: Feature-specific dashboard block or flow component for Подготовка документов; shared status, empty/error, permission, audit, and analytics helpers if present.
- Suggested data objects: DocumentSlot, ExternalReferral, OriginalDocument, StructuredDocumentMetadata.
...
- Suggested path: product/features/doctor-dashboard/15-external-lpu-referral.md
