# Список / выбор других врачей

## 1. Title
- Feature name: Список / выбор других врачей
...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient selects an additional doctor or specialty from the configured list. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 20; Epic = Запись к другим врачам для подготовки; Epic Flow = Пациент выбирает нужного смежного специалиста.; Feature = Список / выбор других врачей; Happy path = Core; Priority = Must-have.
- Normalized feature name: Список / выбор других врачей
...
| Primary behavior | Patient selects an additional doctor or specialty from the configured list. | Patient | User action and relevant object ids or form values. | renders approved internal options and records the patient selection without external scheduling integration. | selected doctor/specialty can be used for a pilot-safe booking request or handoff. |
...
- Upstream step: Patient opens additional doctor options.
- Downstream step: Patient initiates booking or request for the selected option.
...
## 9. Scope
### In scope
- Implement the source feature intent for Список / выбор других врачей inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Список / выбор других врачей.
3. System shows the default state with source/status context and permitted actions.
4. Patient selects an additional doctor or specialty from the configured list.
5. System renders approved internal options and records the patient selection without external scheduling integration and records required analytics/audit events.
6. Patient sees the next state: selected doctor/specialty can be used for a pilot-safe booking request or handoff.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Список / выбор других врачей.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Список / выбор других врачей.
...
- Description: Load Список / выбор других врачей only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient selects an additional doctor or specialty from the configured list.
...
- Description: Record sensitive actions and product funnel events relevant to Список / выбор других врачей.
...
- The feature follows this spec and preserves source row 20 intent.
...
|-------|---------|------------|----------------|
| other_doctors_list_selection_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| other_doctors_list_selection_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| other_doctors_list_selection_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AdditionalDoctorNeed, DoctorOption, SpecialtySelection, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/20-other-doctors-list-selection.md
