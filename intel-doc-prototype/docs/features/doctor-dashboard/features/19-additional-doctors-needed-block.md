# Блок "какие еще врачи могут понадобиться"

## 1. Title
- Feature name: Блок "какие еще врачи могут понадобиться"
...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient views the block listing possible additional doctors for preparation. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 19; Epic = Запись к другим врачам для подготовки; Epic Flow = Пациент понимает, что до основного врача могут понадобиться дополнительные консультации.; Feature = Блок "какие еще врачи могут понадобиться"; Happy path = Core; Priority = Must-have.
- Normalized feature name: Блок "какие еще врачи могут понадобиться"
...
- Expected implementation target: Patient dashboard / additional doctor preparation.
...
| Primary behavior | Patient views the block listing possible additional doctors for preparation. | Patient | User action and relevant object ids or form values. | shows doctor roles explicitly configured by the preparation plan or clinic partner without AI-inferred medical recommendations. | patient sees which additional consultations may be part of preparation. |
...
- Product surface: Patient dashboard / additional doctor preparation.
- Upstream step: Dashboard has a preparation case with configured additional doctor needs.
- Downstream step: Patient opens the list/selection of other doctors.
...
## 9. Scope
### In scope
- Implement the source feature intent for Блок "какие еще врачи могут понадобиться" inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Блок "какие еще врачи могут понадобиться".
3. System shows the default state with source/status context and permitted actions.
4. Patient views the block listing possible additional doctors for preparation.
5. System shows doctor roles explicitly configured by the preparation plan or clinic partner without AI-inferred medical recommendations and records required analytics/audit events.
6. Patient sees the next state: patient sees which additional consultations may be part of preparation.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Блок "какие еще врачи могут понадобиться".
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Блок "какие еще врачи могут понадобиться".
...
- Description: Load Блок "какие еще врачи могут понадобиться" only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient views the block listing possible additional doctors for preparation.
...
- Description: Record sensitive actions and product funnel events relevant to Блок "какие еще врачи могут понадобиться".
...
- The feature follows this spec and preserves source row 19 intent.
...
|-------|---------|------------|----------------|
| additional_doctors_needed_block_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| additional_doctors_needed_block_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| additional_doctors_needed_block_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AdditionalDoctorNeed, PreparationPlan, DoctorSpecialtyOption, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/19-additional-doctors-needed-block.md
