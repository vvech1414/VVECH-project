# Агрегация новых анализов в досье

## 1. Title
- Feature name: Агрегация новых анализов в досье
...
## 2. Purpose
This supporting system behavior supports the patient dashboard preparation journey by letting the patient completes or uploads new analysis results within the plan. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 10; Epic = План новых анализов; Epic Flow = Новые результаты попадают в единый контур подготовки к визиту.; Feature = Агрегация новых анализов в досье; Happy path = Core; Priority = Must-have.
- Normalized feature name: Агрегация новых анализов в досье
...
- Expected implementation target: Patient dashboard / preparation dossier.
...
| Primary behavior | Patient completes or uploads new analysis results within the plan. | Patient | User action and relevant object ids or form values. | adds references to new analysis results into the preparation dossier without merging originals into structured data. | preparation dossier includes new analysis references for permitted doctor review. |
...
- Product surface: Patient dashboard / preparation dossier.
- Upstream step: At least one new analysis result is linked to a plan item.
- Downstream step: Doctor with approved access can review source-backed preparation materials.
...
## 9. Scope
### In scope
- Implement the source feature intent for Агрегация новых анализов в досье inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Агрегация новых анализов в досье.
3. System shows the default state with source/status context and permitted actions.
4. Patient completes or uploads new analysis results within the plan.
5. System adds references to new analysis results into the preparation dossier without merging originals into structured data and records required analytics/audit events.
6. Patient sees the next state: preparation dossier includes new analysis references for permitted doctor review.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Агрегация новых анализов в досье.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Агрегация новых анализов в досье.
...
- Description: Load Агрегация новых анализов в досье only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient completes or uploads new analysis results within the plan.
...
- Description: Record sensitive actions and product funnel events relevant to Агрегация новых анализов в досье.
...
- The feature follows this spec and preserves source row 10 intent.
...
|-------|---------|------------|----------------|
| new_analyses_dossier_aggregation_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| new_analyses_dossier_aggregation_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| new_analyses_dossier_aggregation_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: PreparationDossier, DossierEntry, AnalysisPlanItem, OriginalAnalysisDocument.
...
- Suggested path: product/features/doctor-dashboard/10-new-analyses-dossier-aggregation.md
