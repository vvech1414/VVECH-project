# Статусы новых анализов

## 1. Title
- Feature name: Статусы новых анализов
...
## 2. Purpose
This supporting system behavior supports the patient dashboard preparation journey by letting the patient checks completion status for each new analysis. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 7; Epic = План новых анализов; Epic Flow = Пациент отслеживает выполнение плана подготовки.; Feature = Статусы новых анализов; Happy path = Core; Priority = Must-have.
- Normalized feature name: Статусы новых анализов
...
| Primary behavior | Patient checks completion status for each new analysis. | Patient | User action and relevant object ids or form values. | calculates and displays non-clinical preparation states such as not started, uploaded, pending review, or complete. | patient understands which plan items still need action. |
...
- Upstream step: The new analyses list is loaded.
- Downstream step: Patient uploads missing results or proceeds when required items are complete enough for the pilot flow.
...
## 9. Scope
### In scope
- Implement the source feature intent for Статусы новых анализов inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Статусы новых анализов.
3. System shows the default state with source/status context and permitted actions.
4. Patient checks completion status for each new analysis.
5. System calculates and displays non-clinical preparation states such as not started, uploaded, pending review, or complete and records required analytics/audit events.
6. Patient sees the next state: patient understands which plan items still need action.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Статусы новых анализов.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Статусы новых анализов.
...
- Description: Load Статусы новых анализов only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient checks completion status for each new analysis.
...
- Description: Record sensitive actions and product funnel events relevant to Статусы новых анализов.
...
- The feature follows this spec and preserves source row 7 intent.
...
|-------|---------|------------|----------------|
| new_analysis_statuses_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| new_analysis_statuses_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| new_analysis_statuses_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisPlanItem, AnalysisStatus, ResultAttachment, PreparationReadiness.
...
- Suggested path: product/features/doctor-dashboard/07-new-analysis-statuses.md
