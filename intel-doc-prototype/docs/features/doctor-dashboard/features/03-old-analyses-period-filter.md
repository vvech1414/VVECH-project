# Фильтрация старых анализов по периоду

## 1. Title
- Feature name: Фильтрация старых анализов по периоду
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Старые анализы / Supporting

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient selects a period filter for old analyses. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 3; Epic = Старые анализы; Epic Flow = Пациент быстро находит нужные анализы для подготовки.; Feature = Фильтрация старых анализов по периоду; Happy path = Supporting; Priority = Should-have.
- Normalized feature name: Фильтрация старых анализов по периоду
...
- Expected implementation target: Patient dashboard / old analyses history.
...
| Primary behavior | Patient selects a period filter for old analyses. | Patient | User action and relevant object ids or form values. | filters the patient-owned list by known analysis date or upload date. | old analyses list is narrowed to the selected period. |
...
- Product surface: Patient dashboard / old analyses history.
- Upstream step: Patient sees multiple old analysis records.
- Downstream step: Patient opens a filtered analysis or clears the filter.
...
## 9. Scope
### In scope
- Implement the source feature intent for Фильтрация старых анализов по периоду inside the patient dashboard preparation journey.
...
### Happy path
1. Patient opens the relevant dashboard area for Старые анализы.
2. System validates access to the preparation case and loads only data needed for Фильтрация старых анализов по периоду.
3. System shows the default state with source/status context and permitted actions.
4. Patient selects a period filter for old analyses.
5. System filters the patient-owned list by known analysis date or upload date and records required analytics/audit events.
6. Patient sees the next state: old analyses list is narrowed to the selected period.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Фильтрация старых анализов по периоду.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Фильтрация старых анализов по периоду.
...
- Description: Load Фильтрация старых анализов по периоду only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient selects a period filter for old analyses.
...
- Description: Record sensitive actions and product funnel events relevant to Фильтрация старых анализов по периоду.
...
- The feature follows this spec and preserves source row 3 intent.
...
|-------|---------|------------|----------------|
| old_analyses_period_filter_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| old_analyses_period_filter_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| old_analyses_period_filter_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |

## 16. Suggested implementation notes for Claude Code
- Suggested components/modules: Feature-specific dashboard block or flow component for Старые анализы; shared status, empty/error, permission, audit, and analytics helpers if present.
- Suggested data objects: AnalysisRecord, DateRangeFilter, OriginalAnalysisDocument, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/03-old-analyses-period-filter.md
