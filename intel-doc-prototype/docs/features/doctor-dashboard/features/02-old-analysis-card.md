# Карточка старого анализа

## 1. Title
- Feature name: Карточка старого анализа
...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient opens a selected old analysis. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 2; Epic = Старые анализы; Epic Flow = Пациент открывает конкретный анализ и видит его содержимое и контекст.; Feature = Карточка старого анализа; Happy path = Core; Priority = Must-have.
- Normalized feature name: Карточка старого анализа
...
- Expected implementation target: Patient dashboard / old analysis detail.
...
| Primary behavior | Patient opens a selected old analysis. | Patient | User action and relevant object ids or form values. | shows the original analysis document and any structured metadata as separate source-backed blocks. | analysis details are visible without clinical interpretation. |
...
- Product surface: Patient dashboard / old analysis detail.
- Upstream step: Patient selects an item from the old analyses list.
- Downstream step: Patient returns to the list or uses the analysis as preparation context.
...
## 9. Scope
### In scope
- Implement the source feature intent for Карточка старого анализа inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Карточка старого анализа.
3. System shows the default state with source/status context and permitted actions.
4. Patient opens a selected old analysis.
5. System shows the original analysis document and any structured metadata as separate source-backed blocks and records required analytics/audit events.
6. Patient sees the next state: analysis details are visible without clinical interpretation.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Карточка старого анализа.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Карточка старого анализа.
...
- Description: Load Карточка старого анализа only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient opens a selected old analysis.
...
- Description: Record sensitive actions and product funnel events relevant to Карточка старого анализа.
...
- The feature follows this spec and preserves source row 2 intent.
...
|-------|---------|------------|----------------|
| old_analysis_card_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| old_analysis_card_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| old_analysis_card_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisRecord, OriginalAnalysisDocument, StructuredAnalysisMetadata, SourceAttachment.
...
- Suggested path: product/features/doctor-dashboard/02-old-analysis-card.md
