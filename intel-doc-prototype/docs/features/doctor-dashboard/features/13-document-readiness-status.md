...
## 1. Title
- Feature name: Статус готовности документов
...
## 2. Purpose
This supporting system behavior supports the patient dashboard preparation journey by letting the patient checks whether the document package is ready. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 13; Epic = Подготовка документов; Epic Flow = Пациент видит, насколько пакет документов готов.; Feature = Статус готовности документов; Happy path = Core; Priority = Must-have.
- Normalized feature name: Статус готовности документов
...
| Primary behavior | Patient checks whether the document package is ready. | Patient | User action and relevant object ids or form values. | summarizes required slot completion using logistical readiness states only. | patient sees document readiness and missing required items. |
...
- Upstream step: Document checklist and upload states exist.
- Downstream step: Patient uploads missing documents or proceeds with clear incomplete-state messaging.
...
## 9. Scope
### In scope
- Implement the source feature intent for Статус готовности документов inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Статус готовности документов.
3. System shows the default state with source/status context and permitted actions.
4. Patient checks whether the document package is ready.
5. System summarizes required slot completion using logistical readiness states only and records required analytics/audit events.
6. Patient sees the next state: patient sees document readiness and missing required items.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Статус готовности документов.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Статус готовности документов.
...
- Description: Load Статус готовности документов only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient checks whether the document package is ready.
...
- Description: Record sensitive actions and product funnel events relevant to Статус готовности документов.
...
- The feature follows this spec and preserves source row 13 intent.
...
|-------|---------|------------|----------------|
| document_readiness_status_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| document_readiness_status_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| document_readiness_status_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: DocumentChecklist, DocumentReadiness, DocumentSlot, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/13-document-readiness-status.md
