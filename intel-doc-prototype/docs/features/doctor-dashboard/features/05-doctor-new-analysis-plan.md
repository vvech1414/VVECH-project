# План новых анализов от врача

## 1. Title
- Feature name: План новых анализов от врача
...
## 2. Purpose
This feature supports the patient dashboard preparation journey by letting the patient opens the doctor-created new analyses plan. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 5; Epic = План новых анализов; Epic Flow = Врач формирует план новых анализов, а пациент получает его в приложении.; Feature = План новых анализов от врача; Happy path = Core; Priority = Must-have.
- Normalized feature name: План новых анализов от врача
...
- Expected implementation target: Patient dashboard / new analyses plan.
...
| Primary behavior | Patient opens the doctor-created new analyses plan. | Patient | User action and relevant object ids or form values. | shows the plan items that were explicitly created or approved by a doctor within the preparation case. | patient can see requested preparation analyses and next actions. |
...
- Product surface: Patient dashboard / new analyses plan.
- Upstream step: Doctor or clinic partner creates a preparation plan in an approved IntelDoc workflow.
- Downstream step: Patient reviews the list of new analyses and tracks completion.
...
## 9. Scope
### In scope
- Implement the source feature intent for План новых анализов от врача inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for План новых анализов от врача.
3. System shows the default state with source/status context and permitted actions.
4. Patient opens the doctor-created new analyses plan.
5. System shows the plan items that were explicitly created or approved by a doctor within the preparation case and records required analytics/audit events.
6. Patient sees the next state: patient can see requested preparation analyses and next actions.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for План новых анализов от врача.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for План новых анализов от врача.
...
- Description: Load План новых анализов от врача only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient opens the doctor-created new analyses plan.
...
- Description: Record sensitive actions and product funnel events relevant to План новых анализов от врача.
...
- The feature follows this spec and preserves source row 5 intent.
...
|-------|---------|------------|----------------|
| doctor_new_analysis_plan_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| doctor_new_analysis_plan_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| doctor_new_analysis_plan_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisPlan, AnalysisPlanItem, PreparationCase, DoctorAuthorship.
...
- Suggested path: product/features/doctor-dashboard/05-doctor-new-analysis-plan.md
