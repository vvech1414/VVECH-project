# Переход из подготовки в запись

## 1. Title
- Feature name: Переход из подготовки в запись
...
## 2. Purpose
This flow step supports the patient dashboard preparation journey by letting the patient moves from preparation progress into main doctor booking. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 25; Epic = Запись к основному врачу; Epic Flow = Пациент понимает, что запись является финальным шагом после подготовки.; Feature = Переход из подготовки в запись; Happy path = Core; Priority = Must-have.
- Normalized feature name: Переход из подготовки в запись
...
- Expected implementation target: Patient dashboard / preparation to booking transition.
...
| Primary behavior | Patient moves from preparation progress into main doctor booking. | Patient | User action and relevant object ids or form values. | uses readiness signals to guide transition without blocking solely on clinical interpretation. | patient reaches main booking entry point with preparation status context. |
...
- Product surface: Patient dashboard / preparation to booking transition.
- Upstream step: Patient has reviewed preparation analyses, documents, complaints, and any additional doctor steps.
- Downstream step: Patient starts main doctor booking or returns to incomplete preparation items.
...
## 9. Scope
### In scope
- Implement the source feature intent for Переход из подготовки в запись inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Переход из подготовки в запись.
3. System shows the default state with source/status context and permitted actions.
4. Patient moves from preparation progress into main doctor booking.
5. System uses readiness signals to guide transition without blocking solely on clinical interpretation and records required analytics/audit events.
6. Patient sees the next state: patient reaches main booking entry point with preparation status context.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Переход из подготовки в запись.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Переход из подготовки в запись.
...
- Description: Load Переход из подготовки в запись only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient moves from preparation progress into main doctor booking.
...
- Description: Record sensitive actions and product funnel events relevant to Переход из подготовки в запись.
...
- The feature follows this spec and preserves source row 25 intent.
...
|-------|---------|------------|----------------|
| preparation_to_booking_transition_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| preparation_to_booking_transition_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| preparation_to_booking_transition_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: PreparationReadiness, DashboardState, BookingRequest, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/25-preparation-to-booking-transition.md
