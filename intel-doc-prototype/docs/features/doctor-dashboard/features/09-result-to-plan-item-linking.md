# Привязка результата к пункту плана

## 1. Title
- Feature name: Привязка результата к пункту плана
...
## 2. Purpose
This supporting system behavior supports the patient dashboard preparation journey by letting the patient confirms or relies on the selected plan item during upload. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 9; Epic = План новых анализов; Epic Flow = Система связывает загруженный результат с соответствующим пунктом плана.; Feature = Привязка результата к пункту плана; Happy path = Core; Priority = Must-have.
- Normalized feature name: Привязка результата к пункту плана
...
| Primary behavior | Patient confirms or relies on the selected plan item during upload. | Patient | User action and relevant object ids or form values. | creates an explicit link between the uploaded result and exactly one plan item unless replacement rules are approved. | plan item shows the linked result with traceable source reference. |
...
- Upstream step: A result file has been uploaded from a specific assigned analysis item.
- Downstream step: Result appears in the plan status and preparation dossier aggregation.
...
## 9. Scope
### In scope
- Implement the source feature intent for Привязка результата к пункту плана inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Привязка результата к пункту плана.
3. System shows the default state with source/status context and permitted actions.
4. Patient confirms or relies on the selected plan item during upload.
5. System creates an explicit link between the uploaded result and exactly one plan item unless replacement rules are approved and records required analytics/audit events.
6. Patient sees the next state: plan item shows the linked result with traceable source reference.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Привязка результата к пункту плана.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Привязка результата к пункту плана.
...
- Description: Load Привязка результата к пункту плана only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient confirms or relies on the selected plan item during upload.
...
- Description: Record sensitive actions and product funnel events relevant to Привязка результата к пункту плана.
...
- The feature follows this spec and preserves source row 9 intent.
...
|-------|---------|------------|----------------|
| result_to_plan_item_linking_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| result_to_plan_item_linking_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| result_to_plan_item_linking_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisPlanItem, ResultAttachmentLink, OriginalAnalysisDocument, AuditEvent.
...
- Suggested path: product/features/doctor-dashboard/09-result-to-plan-item-linking.md
