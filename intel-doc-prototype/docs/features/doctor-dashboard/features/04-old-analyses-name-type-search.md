...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient searches old analyses by name or type. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 4; Epic = Старые анализы; Epic Flow = Пациент быстро находит нужные анализы для подготовки.; Feature = Поиск по названию / типу анализа; Happy path = Supporting; Priority = Should-have.
- Normalized feature name: Поиск по названию / типу анализа
...
| Primary behavior | Patient searches old analyses by name or type. | Patient | User action and relevant object ids or form values. | matches against stored labels and structured metadata without inferring medical meaning. | matching old analyses are shown with empty and clear-search states. |
...
- Upstream step: Patient needs a specific prior result from a larger list.
- Downstream step: Patient opens a matching analysis or clears the search.
...
## 9. Scope
### In scope
- Implement the source feature intent for Поиск по названию / типу анализа inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Поиск по названию / типу анализа.
3. System shows the default state with source/status context and permitted actions.
4. Patient searches old analyses by name or type.
5. System matches against stored labels and structured metadata without inferring medical meaning and records required analytics/audit events.
6. Patient sees the next state: matching old analyses are shown with empty and clear-search states.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Поиск по названию / типу анализа.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Поиск по названию / типу анализа.
...
- Description: Load Поиск по названию / типу анализа only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient searches old analyses by name or type.
...
- The feature follows this spec and preserves source row 4 intent.
...
|-------|---------|------------|----------------|
| old_analyses_name_type_search_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| old_analyses_name_type_search_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| old_analyses_name_type_search_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: AnalysisRecord, SearchQuery, StructuredAnalysisMetadata, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/04-old-analyses-name-type-search.md
