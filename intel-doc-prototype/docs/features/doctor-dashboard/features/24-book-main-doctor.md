...
## 1. Title
- Feature name: Запись к основному врачу
...
## 2. Purpose
This flow step supports the patient dashboard preparation journey by letting the patient initiates main doctor booking. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 24; Epic = Запись к основному врачу; Epic Flow = Пациент инициирует запись к основному специалисту.; Feature = Запись к основному врачу; Happy path = Core; Priority = Must-have.
- Normalized feature name: Запись к основному врачу
...
| Primary behavior | Patient initiates main doctor booking. | Patient | User action and relevant object ids or form values. | creates an internal request or approved partner handoff and tracks booking status without assuming live scheduling integration. | main doctor booking request is pending, confirmed, failed, or blocked. |
...
- Upstream step: No main appointment exists or patient is ready to request one.
- Downstream step: Patient sees appointment/request status and preparation context remains linked.
...
## 9. Scope
### In scope
- Implement the source feature intent for Запись к основному врачу inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Запись к основному врачу.
3. System shows the default state with source/status context and permitted actions.
4. Patient initiates main doctor booking.
5. System creates an internal request or approved partner handoff and tracks booking status without assuming live scheduling integration and records required analytics/audit events.
6. Patient sees the next state: main doctor booking request is pending, confirmed, failed, or blocked.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Запись к основному врачу.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Запись к основному врачу.
...
- Description: Load Запись к основному врачу only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient initiates main doctor booking.
...
- Description: Record sensitive actions and product funnel events relevant to Запись к основному врачу.
...
- The feature follows this spec and preserves source row 24 intent.
...
|-------|---------|------------|----------------|
| book_main_doctor_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| book_main_doctor_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| book_main_doctor_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: MainDoctorAppointment, BookingRequest, BookingStatus, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/24-book-main-doctor.md
