# Связка "подготовительный визит -> основной врач"

## 1. Title
- Feature name: Связка "подготовительный визит -> основной врач"
- Source feature name, if different: Same as source.
- Feature layer / epic / product area: Patient dashboard / preparation journey / Запись к другим врачам для подготовки / Core

## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient views the relationship between a preparatory visit and the main doctor visit. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 22; Epic = Запись к другим врачам для подготовки; Epic Flow = Пациент понимает, зачем подготовительный визит нужен в общем сценарии подготовки.; Feature = Связка "подготовительный визит -> основной врач"; Happy path = Core; Priority = Must-have.
- Normalized feature name: Связка "подготовительный визит -> основной врач"
...
- Assumptions made: MVP uses IntelDoc-owned preparation records, patient-controlled access, and internal/mocked service boundaries. External scheduling, clinic calendars, EHR booking APIs, payment, and insurance systems are future dependency / out of scope for MVP unless explicitly approved.
...
- Expected implementation target: Patient dashboard / preparation-to-main-visit context.
...
- Minimal integration assumptions: External scheduling, clinic calendars, EHR booking APIs, payment, and insurance systems are future dependency / out of scope for MVP unless explicitly approved.
...
- Auditability expectations: Views of sensitive patient data by doctors/admins must be logged; patient self-view should be product analytics unless project policy requires audit.
...
- Original vs structured data handling, if relevant: No original-vs-structured data split is required unless this feature displays source documents or derived metadata.
...
| Primary behavior | Patient views the relationship between a preparatory visit and the main doctor visit. | Patient | User action and relevant object ids or form values. | shows a logistical sequence link between preparation consultation and main appointment without clinical necessity claims. | patient understands how the preparatory visit fits into the booking flow. |
...
- Product surface: Patient dashboard / preparation-to-main-visit context.
- Upstream step: An additional doctor need or booking request exists.
- Downstream step: Patient continues preparation or proceeds toward main doctor booking.
...
## 9. Scope
### In scope
- Implement the source feature intent for Связка "подготовительный визит -> основной врач" inside the patient dashboard preparation journey.
...
### Happy path
1. Patient opens the relevant dashboard area for Запись к другим врачам для подготовки.
2. System validates access to the preparation case and loads only data needed for Связка "подготовительный визит -> основной врач".
3. System shows the default state with source/status context and permitted actions.
4. Patient views the relationship between a preparatory visit and the main doctor visit.
5. System shows a logistical sequence link between preparation consultation and main appointment without clinical necessity claims and records required analytics/audit events.
6. Patient sees the next state: patient understands how the preparatory visit fits into the booking flow.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Связка "подготовительный визит -> основной врач".
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Связка "подготовительный визит -> основной врач".
...
- Description: Load Связка "подготовительный визит -> основной врач" only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient views the relationship between a preparatory visit and the main doctor visit.
- Rules: Keep the behavior limited to the source row intent and current preparation context. External scheduling, clinic calendars, EHR booking APIs, payment, and insurance systems are future dependency / out of scope for MVP unless explicitly approved.
...
- Description: Record sensitive actions and product funnel events relevant to Связка "подготовительный визит -> основной врач".
...
- The feature follows this spec and preserves source row 22 intent.
...
|-------|---------|------------|----------------|
| preparatory_visit_main_doctor_link_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| preparatory_visit_main_doctor_link_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| preparatory_visit_main_doctor_link_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |

## 16. Suggested implementation notes for Claude Code
- Suggested components/modules: Feature-specific dashboard block or flow component for Запись к другим врачам для подготовки; shared status, empty/error, permission, audit, and analytics helpers if present.
- Suggested data objects: PreparationStepLink, BookingRequest, MainDoctorAppointment, PreparationCase.
...
- Suggested path: product/features/doctor-dashboard/22-preparatory-visit-main-doctor-link.md
