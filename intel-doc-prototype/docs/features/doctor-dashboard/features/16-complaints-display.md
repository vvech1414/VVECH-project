...
## 2. Purpose
This screen section/block supports the patient dashboard preparation journey by letting the patient views current complaints captured for preparation. It exists so the patient can understand preparation progress before the main doctor visit while keeping sensitive health data access-scoped and audit-ready.

## 3. Source normalization notes
- Source row/block: row 16; Epic = Жалобы; Epic Flow = Пациент открывает блок жалоб и фиксирует текущее состояние перед приемом.; Feature = Отображение жалоб; Happy path = Core; Priority = Must-have.
- Normalized feature name: Отображение жалоб
...
| Primary behavior | Patient views current complaints captured for preparation. | Patient | User action and relevant object ids or form values. | renders patient-entered complaint text and metadata as self-reported preparation information. | current complaints are visible with edit/access controls as permitted. |
...
- Upstream step: Patient opens the complaints block on the dashboard.
- Downstream step: Patient edits complaints or adds tags/categories where supported.
...
## 9. Scope
### In scope
- Implement the source feature intent for Отображение жалоб inside the patient dashboard preparation journey.
...
2. System validates access to the preparation case and loads only data needed for Отображение жалоб.
3. System shows the default state with source/status context and permitted actions.
4. Patient views current complaints captured for preparation.
5. System renders patient-entered complaint text and metadata as self-reported preparation information and records required analytics/audit events.
6. Patient sees the next state: current complaints are visible with edit/access controls as permitted.
...
- Primary actions: Expose only actions that are allowed by current permissions and prerequisites for Отображение жалоб.
...
## 12. States
### Default
- Show current feature data, source/status indicators, and permitted primary/secondary actions for Отображение жалоб.
...
- Description: Load Отображение жалоб only for the current patient-owned or otherwise approved preparation case.
...
- Description: Support the core behavior where the patient views current complaints captured for preparation.
...
- The feature follows this spec and preserves source row 16 intent.
...
|-------|---------|------------|----------------|
| complaints_display_viewed | Patient opens the feature surface after access validation. | feature_id, source_row, epic, priority, actor_type, has_existing_data | Reach, empty rate, and error rate are measurable. |
| complaints_display_primary_action | Patient completes the primary action for this feature. | feature_id, source_row, action_type, result_state, preparation_case_id_hash | Patient progresses through the preparation journey. |
| complaints_display_blocked | System blocks access or disables the primary action. | feature_id, source_row, reason_code, actor_type, surface | Permission or prerequisite issues are visible without exposing sensitive data. |
...
- Suggested data objects: ComplaintEntry, ComplaintCategory, PreparationCase, AuditEvent.
...
- Suggested path: product/features/doctor-dashboard/16-complaints-display.md
