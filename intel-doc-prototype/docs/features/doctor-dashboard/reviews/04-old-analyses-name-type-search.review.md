...
## Critical issues
1. Search matching behavior is too ambiguous
- Severity: Medium
- Where: Sections 7, 10, 11, 12, 13, and 15.
- Problem: The spec does not define minimum query length, case/locale normalization, debounce behavior, empty-query reset, highlighted fields, or no-results behavior.
- Why it matters: Search behavior affects performance, accessibility, and user trust when analysis names/types are inconsistent or transliterated.
- Recommended fix: Specify query normalization, supported fields, reset/no-results states, and event properties.
...
## Missing pieces
- Minimum query behavior; locale/case handling; debounce/loading behavior; no-results state; analytics for query_length and result_count.
...
### Patch 1
```md
Add to FR2: Search must be local to the already access-scoped old analyses dataset for MVP. Normalize case and surrounding whitespace, search title, document name, and analysis type, reset results when the query is empty, and show a no-results state with clear action. Do not search external systems or mutate source records.
```
