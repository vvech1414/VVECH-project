...
- The spec tries to explain the relationship without clinical claims.
...
## Critical issues
1. Link semantics between preparatory and main visit are incomplete
- Severity: Medium
- Where: Sections 7, 10, 11, 12, 13, and 16.
- Problem: The spec does not define what creates the link, whether links are one-to-many, who can remove them, or what data crosses from the preparatory visit to the main doctor context.
- Why it matters: This link controls navigation and possible data sharing; undefined semantics can leak context or confuse visit readiness.
- Recommended fix: Define link creation, cardinality, visible metadata, unlink rules, and data-sharing boundaries.
...
## Missing pieces
- Link creation trigger; cardinality; unlink behavior; data shared through link; audit event; stale/cancelled linked visit state.
...
### Patch 1
```md
Add to FR2: A PreparationVisitLink must connect one related specialist appointment/request to one preparation case for the main doctor unless product approves many-to-many. Show link source, linked appointment/request status, and last updated timestamp. Creating/removing the link must be explicit and audited. The link carries identifiers and status only; document or complaint originals are not shared through the link without separate access rules.
```
