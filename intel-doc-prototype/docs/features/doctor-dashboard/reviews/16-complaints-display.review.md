...
- The spec frames complaints as patient-provided context, not clinical conclusions.
...
## Critical issues
1. Complaint display source/version rules are underdefined
- Severity: Medium
- Where: Sections 7, 10, 11, 12, 13, and 14.
- Problem: The spec does not define what fields are displayed, whether historical versions are visible, how empty/deleted complaints appear, or how doctor view differs from patient view.
- Why it matters: Complaints are sensitive patient-entered content; source/version clarity matters for doctor preparation and auditability.
- Recommended fix: Define display fields, current-vs-history behavior, deleted/empty states, and role-specific visibility.
...
## Missing pieces
- Displayed fields; version/history policy; deleted/empty complaint handling; role-specific read/write rules; audit for doctor view.
...
### Patch 1
```md
Add to FR2: Complaint display must show the current patient-entered text, selected non-diagnostic tags if present, last edited timestamp, and source label "patient-provided". Historical versions are out of scope unless explicitly supported; if supported, they must be read-only and auditable. Doctor view must be read-only for patient-entered complaint text in MVP.
```
