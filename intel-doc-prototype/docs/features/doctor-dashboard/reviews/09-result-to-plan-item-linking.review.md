...
## Critical issues
1. Linking rules do not cover conflicts or unlinking
- Severity: Medium
- Where: Sections 7, 10, 12, 13, and 14.
- Problem: The spec does not define one-result vs many-results per plan item, conflict resolution, unlink behavior, validation criteria, or how wrong-item corrections are audited.
- Why it matters: Linking determines readiness and dossier inclusion; ambiguous cardinality or correction rules can corrupt preparation state.
- Recommended fix: Define cardinality, validation, relink/unlink behavior, and audit events.
...
## Missing pieces
- Cardinality; same-case validation; relink/unlink behavior; conflict state; audit event for correction.
...
### Patch 1
```md
Add to FR2: Linking must validate that the upload and plan item belong to the same preparation case and accessible patient. MVP should allow one active result per plan item unless explicitly configured otherwise. Replacing or unlinking an active result requires an explicit action, records audit metadata, and updates the plan item status without deleting the original upload record.
```
