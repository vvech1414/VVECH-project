...
- Summary: Structurally complete 17-section feature spec for `Состояние “основная запись отсутствует”` and aligned with the broad IntelDoc guardrails. It is not fully implementation-ready because key behavior remains templated/generic, especially feature-specific state logic, access lifecycle, audit details, and analytics acceptance criteria.

## What is strong
- The artifact type is clear enough for review as a state/block and the file follows the canonical section structure.
- The spec identifies absence of a main appointment as a distinct state with booking CTA.
...
## Critical issues
1. Absent-state criteria and CTA gating are unclear
- Severity: Medium
- Where: Sections 7, 10, 11, 12, 13, and 14.
- Problem: The spec does not define what counts as no main appointment, how pending/rejected/cancelled requests differ, when the CTA is enabled, or whether preparation readiness affects booking.
- Why it matters: This state can incorrectly prompt duplicate booking or block a valid user path.
- Recommended fix: Define appointment/request states, CTA enablement, duplicate prevention, and readiness copy.
...
## Missing pieces
- Definition of absent; pending/rejected/cancelled handling; CTA gating; duplicate prevention; readiness dependency.
...
### Patch 1
```md
Add to FR2: Show the absent state only when there is no confirmed main appointment and no active submitted/pending main appointment request for the preparation case. If a pending request exists, show pending state instead of booking CTA. If a prior request was rejected or cancelled, allow a new CTA with clear status copy. Preparation readiness may inform copy but must not block booking unless product explicitly configures it.
```
