...
- The spec introduces concrete status labels and computes state from plan/upload/link data.
...
## Critical issues
1. Status state machine is incomplete
- Severity: Medium
- Where: Sections 7, 10, 12, 13, and 14.
- Problem: Statuses are named but allowed transitions, source-of-truth precedence, manual overrides, and failure/review states are not defined.
- Why it matters: Status ambiguity will create inconsistent readiness calculations and unclear patient next actions.
- Recommended fix: Define a deterministic status model with allowed transitions and blocked/review states.
...
## Missing pieces
- Allowed statuses; transition table; manual override policy; source-of-truth precedence; failed upload/review state.
...
### Patch 1
```md
Add to FR2: Each plan item status must be computed deterministically as not_started, upload_in_progress, uploaded_unlinked, linked, needs_attention, or unavailable. The source of truth is plan item plus upload/link records; patient manual status changes are out of scope unless separately approved. Failed uploads return to the previous stable status and preserve traceable upload attempts.
```
