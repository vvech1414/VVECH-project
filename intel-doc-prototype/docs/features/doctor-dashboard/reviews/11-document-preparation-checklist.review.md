...
- The checklist intent is clear and remains integration-light.
...
## Critical issues
1. Checklist item model and readiness logic are not defined
- Severity: Medium
- Where: Sections 7, 10, 11, 12, 13, and 14.
- Problem: The spec does not define required vs optional documents, item statuses, ordering, who configures the checklist, or how readiness is computed from item state.
- Why it matters: Without concrete checklist rules, implementation cannot reliably show what is missing or ready before booking.
- Recommended fix: Define checklist item schema, status set, ordering/configuration source, and readiness dependencies.
...
## Missing pieces
- Checklist item schema; required/optional rules; item status set; configuration source; readiness computation.
...
### Patch 1
```md
Add to FR2: A checklist item must include item type, display label, required/optional flag, current attachment state, last updated timestamp, and allowed actions. MVP checklist configuration should be internal/static or clinician-configured; no external document-system sync. Readiness is computed only from required item states unless product explicitly opts into optional blockers.
```
