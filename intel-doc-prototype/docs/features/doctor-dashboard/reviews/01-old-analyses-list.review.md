...
- The list intent, stable newest-first ordering, and separation of original analysis documents from structured fields are stated.
...
## Critical issues
1. List behavior is still too generic for implementation
- Severity: Medium
- Where: Sections 7, 10, 11, 12, and 13; especially the primary action row and FR2.
- Problem: The spec says to show a scannable list, but does not define minimum columns, sort tie-breakers, document availability indicators, pagination/loading behavior, or how list selection leads to the analysis card.
- Why it matters: A builder must guess the data contract and list UX, which can create inconsistent handling of old results and originals.
- Recommended fix: Define the minimum list item fields, sort order, empty/list density behavior, and click-through target to the old analysis card.
...
## Missing pieces
- Minimum list item fields; sort tie-breakers; pagination or lazy-load behavior; click-through target; patient-visible original-vs-extracted indicators.
...
### Patch 1
```md
Add to FR2: The old analyses list must render one row/card per accessible AnalysisRecord with display title, analysis type when known, result date, upload date fallback, source/original availability, and structured extraction status. Sort by result date descending, then upload date descending, then created id for stability. Selecting an item opens the old analysis card for that record without mutating source data.
```
