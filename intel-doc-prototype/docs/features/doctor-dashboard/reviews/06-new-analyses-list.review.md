...
- The spec ties the patient list to doctor-created plan items and upload affordances.
...
## Critical issues
1. Patient-facing plan list lacks item-level display and action rules
- Severity: Medium
- Where: Sections 7, 10, 11, 12, and 13.
- Problem: The list does not define required item fields, grouping/sort order, status badges, disabled upload conditions, or what happens when a plan is unpublished or changed.
- Why it matters: Patients need a clear task list; ambiguous list behavior can hide required preparation work or expose stale plan data.
- Recommended fix: Define item fields, status badges, sort/grouping, plan-change handling, and upload CTA visibility.
...
## Missing pieces
- Published-only rule; item fields; sort/grouping; stale plan update behavior; upload CTA disable reasons.
...
### Patch 1
```md
Add to FR2: The new analyses list must render published plan items only, showing item name, source doctor, required/optional flag when available, current completion status, and upload/link action where permitted. Sort by plan order from the doctor, then creation time. If the plan changes while viewed, refresh with a non-destructive update notice and do not discard in-progress uploads.
```
